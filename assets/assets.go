...
var (
	...
	fonts          = map[FontName]*text.GoTextFaceSource{}
...
func (f FontName) Get() *text.GoTextFaceSource {
	return getFontSource(f)
}

func (f FontName) GetFontFace(size float64) text.Face {
...
func LoadFont(name FontName, ttf []byte) {
	...
	if err != nil {
		panic(fmt.Sprintf("Could not parse Font %s", name))
	}
	fonts[name] = s
}

func getFontSource(name FontName) *text.GoTextFaceSource {
	f, ok := fonts[name]
	if !ok {
